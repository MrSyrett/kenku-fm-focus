...
const createWindow = (): BrowserWindow => {
  ...
  // F11 toggles distraction-free focus mode. Caught here for when our own UI has
  // keyboard focus; each embedded web view catches it too (see
  // BrowserViewManagerMain) so it works even while the site is focused.
  mainWindow.webContents.on("before-input-event", (event, input) => {
    if (input.type === "keyDown" && input.key === "F11") {
      mainWindow.webContents.send("FOCUS_MODE_TOGGLE");
